...
class SEIRimport(SEIR0): #impotrerer SEIR som en subklasse
    def __init__(self,b,r_ia = 0.1 , r_e2 = 1.55, lmda1 = 0.33, lmda2 = 0.5, p_a = 0.4, mu = 0.2,sigma = 10):
        ...
        self.sigma = sigma #sigma markeerer at det kommer mennesker utenifra med smitte
        if isinstance(b,(float,int)):
            self.b_f = lambda t: b #gjør beta-verdien om til en funksjon, dersom den ikke allerede er kallbar
        elif callable(b):
            self.b_f = b
...
def solve_SEIR0(model,T,N,S_0, E2_0,beta):
    u0 = [S_0,0,E2_0,0,0,0]
    f = SEIRimport(beta) #gir oss derivertverdiene ved å lgae en instns av subklassen
...
t,u = solve_SEIR0(Euler,900,1000,5.5*10**6,100,n) #henter ut verdiene fra SOLVE_SEIR

plot_SEIR(t,u,["Ia","I"]) #bruker plotfunksjonen til å visualsiere dette